...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--fighter-reference-root",
        default=None,
        type=Path,
        help=(
            "Root folder containing named fighter reference folders and an optional exclude folder. "
            "When set, --fighter-a-reference-images defaults to <root>/<fighter-a-name> and "
            "--fighter-a-exclude-reference-images defaults to <root>/exclude."
        ),
    )
    parser.add_argument(
        "--fighter-a-start",
    ...
    parser.add_argument("--fighter-a-min-red-glove-score", default=0.08, type=float)
...
def fighter_reference_folder_name(name: str) -> str:
    return name.strip().lower().replace(" ", "-")


def resolve_fighter_reference_dir(root: Path, fighter_name: str) -> Path:
    preferred = root / fighter_reference_folder_name(fighter_name)
    if preferred.exists():
        return preferred
    normalized = fighter_reference_folder_name(fighter_name)
    if root.exists():
        for candidate in root.iterdir():
            if candidate.is_dir() and fighter_reference_folder_name(candidate.name) == normalized:
                return candidate
    return preferred


def resolve_named_fighter_args(args: argparse.Namespace) -> None:
    root = args.fighter_reference_root or Path("reference")
    args.fighter_reference_root = root
    if not args.fighter_a_reference_images:
        fighter_dir = resolve_fighter_reference_dir(root, args.fighter_a_name)
        if fighter_dir.exists():
            args.fighter_a_reference_images = [fighter_dir]
    if not args.fighter_a_exclude_reference_images:
        exclude_dir = root / "exclude"
        if exclude_dir.exists():
            args.fighter_a_exclude_reference_images = [exclude_dir]
...
def analyze(args: argparse.Namespace) -> dict[str, Any]:
    resolve_named_fighter_args(args)
    ...
    named_counts_per_60 = {
        key: round(value / duration_minutes, 3) if duration_minutes else 0.0
        for key, value in named_counts.items()
    }
    summary = {
        ...
        "fighter_name": args.fighter_a_name,
        "fighter_reference_root": str(args.fighter_reference_root),
        "fighter_a_cues": {
        ...
        "named_fighter_candidate_counts_per_60_seconds": named_counts_per_60,
        "strike_counter": {
            "history_frames": args.strike_history_frames,
            "min_punch_endpoint_motion": args.min_punch_endpoint_motion,
            "min_kick_endpoint_motion": args.min_kick_endpoint_motion,
